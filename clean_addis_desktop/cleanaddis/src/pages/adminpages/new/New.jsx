import "./new.scss"
import CityadminSidebar from "../../../components/cityadmincomponents/sidebar/CityadminSidebar"
import CityadminNavbar from "../../../components/cityadmincomponents/navbar/CityadminNavbar"
import DriveFolderUploadOutlinedIcon from '@mui/icons-material/DriveFolderUpload';

const New = () => {
  return (
    <div className="new flex">
    <CityadminSidebar />    
      <div className="newcontainer pt-3 pl-2 flex-6 w-full">
        <CityadminNavbar />
      <div className = "top flex font-bold text-3xl shadow-xl pt-7 m-2 w-full text-gray-400">
        <h1 >Add new user</h1>
      </div>
      <div className = "bottom flex shadow-xl  m-10 pt-14 pb-14 ">
      <div className = "left items-center">
        <img src="https://images.unsplash.com/photo-1559308078-88465deb35cc?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=870&q=80" alt="" className="w-28 h-28 rounded-full object-cover " />
      </div>
        <div className = "right">
          <form className="formitems-center flex flex-wrap  gap-4">
          <div className="formInput">
            Image : <label htmlFor="file" className="cursor-pointer"> <DriveFolderUploadOutlinedIcon /></label>
            <input type="file" id="file" style={{display:"none"}}/>
            </div>

            <div className="formInput ">
            <label className="items-center"> Title</label>
            <input type="text" placeholder="cleaning" className="border-b-4"/>
            </div>

            <div className="formInput">
            <label>from date:</label>
            <input type="date" className="border-b-4" />
            </div>

            <div className="formInput">
            <label>to date:</label>
            <input type="date" className="border-b-4"/>
            </div>

            <div className="formInput">
            <label>for:</label>
            <input type="text" placeholder="Bole subcity" className="border-b-4" />
            </div>
            <div>
            <button className=" border-solid border-4 border-green-400 items-center ml-5 rounded-full  hover:text-white hover:bg-green-400 ">Create</button>
            </div>
          </form>
        </div>
      </div>
      </div>
    </div>
  )
}

export default New
